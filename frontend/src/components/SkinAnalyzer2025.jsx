...
const SkinAnalyzer2025 = () => {
  ...
  // const overlayCanvasRef = useRef(null); // 사용하지 않음
  ...
  const checkFaceDetection = useCallback(async () => {
    ...
    if (!videoRef.current || !canvasRef.current || !cameraActive) {
      console.log('❌ 얼굴 감지 체크 조건 불만족');
      return;
    }
    
    // 카운트다운 중에도 얼굴 감지 계속 수행 (얼굴이 가려지면 카운트다운 중단 위해)

    const video = videoRef.current;
    
    // 비디오가 준비되지 않은 경우 얼굴 감지를 시도하지 않음
    if (video.videoWidth === 0 || video.videoHeight === 0) {
      console.log('🔍 비디오가 아직 준비되지 않았습니다. 크기:', { 
    ...
    if (video.paused || video.ended || video.readyState < 3) {
      console.log('🔍 비디오가 재생 중이 아닙니다. 상태:', {
        paused: video.paused,
        ended: video.ended,
        readyState: video.readyState
      });
    ...
    try {
      ...
      // 얼굴 감지 상태가 변경되었을 때만 처리
      if (newFaceDetected !== faceDetected) {
        console.log('🔄 얼굴 감지 상태 변경:', {
          이전: faceDetected,
          현재: newFaceDetected,
          카운트다운: countDown
        });
        ...
        if (newFaceDetected && !countDown) {
          ...
          // 비디오가 완전히 준비된 상태에서만 카운트다운 시작
          if (video.videoWidth > 0 && 
              video.videoHeight > 0 && 
              !video.paused && 
              !video.ended && 
              video.readyState >= 3) {
            console.log('🚀 모든 조건 만족, 카운트다운 시작!');
            startCountDown();
          } else {
            console.log('⏳ 비디오가 완전히 준비되지 않아 카운트다운을 연기합니다.', {
        ...
        // 상태 변화 없음 - 로그 스팸 방지
        console.log('📊 얼굴 감지 상태 유지:', {
          얼굴감지: newFaceDetected,
          카운트다운: countDown
        });
      }
      
      // 카운트다운 중 얼굴이 감지되지 않으면 카운트다운 중단 및 재시작 대기
      if (!newFaceDetected && countDown) {
        console.log('⚠️ 카운트다운 중 얼굴 감지 실패! 카운트다운 중단하고 재시작 대기');
        if (countDownInterval.current) {
          clearInterval(countDownInterval.current);
          countDownInterval.current = null;
        }
        setCountDown(null);
        console.log('🛑 카운트다운 완전히 중단됨');
        // 얼굴이 다시 감지되면 자동으로 새로운 카운트다운이 시작됨
      }
      ...
      // 연속 에러 발생 시 얼굴 감지 인터벌 중단
      if (faceCheckInterval.current) {
        console.log('❌ 에러로 인해 얼굴 감지 인터벌 중단');
        clearInterval(faceCheckInterval.current);
        faceCheckInterval.current = null;
        
        // 3초 후 재시도
        setTimeout(() => {
          if (cameraActive && !countDown) {
            console.log('🔄 얼굴 감지 재시도');
            if (!faceCheckInterval.current) {
  ...
  }, [API_BASE_URL, cameraActive, faceDetected, countDown, startCountDown, countDownInterval]);
  ...
  }, [cameraActive, faceDetected, countDown]);
          ...
          {currentStep === 'result' && analysisResult && (
            <div>
              ...
              <div className="bg-white/80 backdrop-blur-lg rounded-3xl shadow-2xl border border-white/20 p-6 mb-8">
                ...
                <div className="relative rounded-2xl overflow-hidden shadow-lg bg-gray-100 max-w-2xl mx-auto">
                  <img 
                    ref={imageRef}
                    src={capturedImage}
                    alt="분석된 이미지"
                  ...
                  {analysisResult.acne_lesions && analysisResult.acne_lesions.map((lesion, index) => {
                    ...
                    if (lesion.normalized_x !== undefined && lesion.normalized_y !== undefined) {
                      ...
                      if (imgElement) {
                        // 간단하고 정확한 좌표 계산
                        const displayedWidth = imgElement.clientWidth || imgElement.offsetWidth;
                        const displayedHeight = imgElement.clientHeight || imgElement.offsetHeight;
                        
                        left = lesion.normalized_x * displayedWidth;
                        top = lesion.normalized_y * displayedHeight;
                        width = (lesion.normalized_width || 0.02) * displayedWidth;
                        height = (lesion.normalized_height || 0.02) * displayedHeight;
                      } else {
                        // 백업: 픽셀 좌표 사용
                        left = lesion.x * scale.x;
                        top = lesion.y * scale.y;
                        width = lesion.width * scale.x;
                        height = lesion.height * scale.y;
                      }
                    } else {
                    ...
                    >
                      <div className="absolute -top-5 left-0 bg-red-500 text-white text-xs px-1 py-0.5 rounded text-center" style={{fontSize: '10px'}}>
