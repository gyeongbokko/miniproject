...
const AcneMarker = ({ 
  ...
  const renderMarker = useCallback((lesion, index) => {
    ...
    // 스케일링 비율 계산 (표시 크기 대비 원본 크기)
    const scaleX = displayedWidth / originalWidth;
    const scaleY = displayedHeight / originalHeight;
    
    // 백엔드에서 받은 픽셀 좌표에 스케일링 적용
    const x = lesion.x * scaleX;
    const y = lesion.y * scaleY;
    const width = Math.max((lesion.width || 12) * scaleX, 8);
    const height = Math.max((lesion.height || 12) * scaleY, 8);
