...
from roboflow_detector import AcneDetector
import json
...
class ModernSkinAnalyzer:
    def __init__(self):
        ...
        # Roboflow 여드름 감지 모델 초기화
        try:
            self.acne_detector = AcneDetector()
            if self.acne_detector.is_available():
                logger.info("✨ Roboflow 여드름 감지 모델 로드 완료!")
            else:
                logger.warning("⚠️ Roboflow API 초기화 실패. 환경변수를 확인해주세요.")
        except Exception as e:
            logger.error(f"❌ Roboflow 모델 로드 실패: {e}")
            self.acne_detector = None
        
        self.min_face_confidence = 0.1  # 매우 낮은 임계값으로 얼굴 감지 향상
    ...
    def detect_faces(self, image: np.ndarray) -> List[Dict]:
        ...
        try:
            ...
            faces = self.face_cascade.detectMultiScale(
                gray,
                scaleFactor=1.05,  # 더 민감한 감지
                minNeighbors=3,    # 낮은 민감도로 더 많은 얼굴 감지
...
IMPORTANT: 반드시 JSON 형태로만 응답하세요. 설명문이나 추가 텍스트 없이 순수 JSON만 출력하세요. 첫 글자는 {{로 시작하고 마지막 글자는 }}로 끝나야 합니다."""
    ...
    async def analyze_image_advanced(self, image: np.ndarray, chatbot_data: Dict[str, Any] = None) -> SkinAnalysisResult:
        ...
        try:
            ...
            # 8. 여드름 감지 (원본 이미지에서 직접 감지 - 좌표 변환 불필요)
            acne_lesions = self.detect_acne_advanced(image, None)  # 원본 이미지 사용, bbox 없음
            
            # 여드름 좌표를 cho-2 프로젝트 방식으로 간단 처리
            adjusted_acne_lesions = acne_lesions  # 좌표 변환 없이 그대로 사용
            
            logger.info(f"🔄 원본 이미지에서 직접 감지: {len(acne_lesions)}개 발견")
            
            for i, lesion in enumerate(adjusted_acne_lesions):
                logger.info(f"🎯 여드름 #{i+1}: ({lesion['x']},{lesion['y']}) conf={lesion['confidence']:.2f}")
    ...
    def detect_acne_advanced(self, image: np.ndarray, bbox: Dict = None) -> List[Dict]:
        """Roboflow API를 사용한 고급 여드름 감지"""
        acne_lesions = []
        
        # 1차: Roboflow API 감지
        if self.acne_detector and self.acne_detector.is_available():
            try:
                logger.info("🔍 Roboflow API 여드름 감지 시작...")
                
                # Roboflow API 호출 (cho-2 프로젝트와 정확히 동일한 설정)
                detected_lesions = self.acne_detector.detect_acne_roboflow(image)
                
                # cho-2 방식: 얼굴 영역 필터링 없이 모든 감지 결과 사용
                for lesion in detected_lesions:
                    lesion["detection_method"] = "roboflow"
                acne_lesions = detected_lesions
                
                logger.info(f"✅ Roboflow API 감지: {len(acne_lesions)}개 발견")
                
            except Exception as e:
                logger.error(f"❌ Roboflow API 여드름 감지 오류: {str(e)}")
        else:
            logger.warning("⚠️ Roboflow API를 사용할 수 없습니다. 환경변수를 확인해주세요.")
        
        # OpenCV 보완 감지 비활성화 (cho-2 프로젝트와 동일)
...
# LangChain/LangGraph 통합 임포트 (현재 비활성화)
try:
    # from langchain_integration import analyze_skin_with_langchain
    LANGCHAIN_AVAILABLE = False
    logger.info("⚠️ LangChain/LangGraph 모듈을 로드할 수 없습니다: 임시 비활성화")
